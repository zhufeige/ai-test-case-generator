...
export const EditTestCaseModal: React.FC<EditTestCaseModalProps> = ({ testCase, onClose }) => {
  ...
  const handleSave = () => {
    updateTestCase(testCase.id, form);
    onClose();
  };
    ...
    <div className="fixed inset-0 bg-black/50 flex items-center justify-center z-[60]" onClick={onClose}>
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-2xl mx-4 max-h-[90vh] flex flex-col" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200">
          <h3 className="text-lg font-semibold text-gray-800">编辑测试用例</h3>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4 space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <FieldInput label="标题" value={form.title} onChange={(v) => handleChange('title', v)} />
            <FieldInput label="模块" value={form.module} onChange={(v) => handleChange('module', v)} />
        ...
        <div className="flex items-center justify-end gap-3 px-6 py-4 border-t border-gray-200">
          ...
          <button
            onClick={handleSave}
            className="flex items-center gap-2 px-5 py-2 bg-blue-500 text-white rounded-lg text-sm font-medium hover:bg-blue-600 transition-colors"
          >
